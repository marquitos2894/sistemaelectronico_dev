...
Class almacenControlador extends almacenModelo {
    ...
    public function paginador_componentes($paginador,$registros,$privilegio,$buscador,$vista){
        ...
        if($buscador!=""){
            $datos=$conexion->prepare("SELECT c.id_comp,c.codigo,c.descripcion,c.nparte1,c.nparte2,c.nparte3,c.unidad_med,ac.stock,ac.fk_idalm,ac.id_ac,ac.u_nombre,ac.u_seccion
                                     FROM componentes c
    ...
    public function save_vsalida_controlador(){
        ...
        if($id_vsalida!=0){
            ...
            mainModel::ejecutar_consulta_simple("UPDATE vale_salida SET est = 0 WHERE id_vsalida = {$id_vsalida}");                       
                ...
                $datos = ["tipo"=>"success","mensaje"=>"Vale N°{$id_vsalida} generado con exito !! haga click aqui, o recargar la pagina para actualizar la informacion "];
    ...
    public function obtener_consulta_json_controlador(){
        $consulta = "SELECT ac.id_ac,c.id_comp,c.codigo,c.descripcion,c.nparte1,c.nparte2,c.nparte3,c.unidad_med,ac.stock,ac.fk_idalm,ac.u_nombre,ac.u_seccion
                    FROM componentes c
                    INNER JOIN almacen_componente ac
